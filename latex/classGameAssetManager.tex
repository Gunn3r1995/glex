...
22   \};
23 
24   \hyperlink{classGameAssetManager_ad7bab17862e06ca692289f934b40548b}{program\_token} = \hyperlink{classGameAssetManager_abec45b44a8b35ad2d7d817ba10e0dd8d}{CreateGLProgram}(vertex\_shader, fragment\_shader);
25 \}
\end{DoxyCode}
...
\begin{DoxyCode}
31                                     \{
32   glDeleteProgram(\hyperlink{classGameAssetManager_ad7bab17862e06ca692289f934b40548b}{program\_token});
33 \}
\end{DoxyCode}
...
\begin{DoxyCode}
39                                                                       \{
40   \textcolor{comment}{// TODO: implement this}
41 \}
\end{DoxyCode}
...
\begin{DoxyCode}
47                                                                        \{
48   \textcolor{comment}{// TODO: implement this}
49 \}
\end{DoxyCode}
...
\begin{DoxyCode}
62                                                                   \{
63   \hyperlink{classGameAssetManager_a671cddd92f1de4186c582fe0c4297b7d}{draw\_list}.push\_back(the\_asset);
64 \}
\end{DoxyCode}
...
\begin{DoxyCode}
111                                                                          \{
112   GLuint shader\_token;
113   GLint shader\_ok;
114   \textcolor{keyword}{auto} source = \hyperlink{classGameAssetManager_a23b124a213308a68a882727127601c97}{ReadShader}(shader);
115 
116   \textcolor{keywordflow}{if} (!source.first)
117     \textcolor{keywordflow}{return} 0;
118 
119   shader\_token = glCreateShader(type);
120   glShaderSource(shader\_token, 1, (\textcolor{keyword}{const} GLchar**)&source.first, &source.second);
121   glCompileShader(shader\_token);
122   \textcolor{keyword}{delete}(source.first);
123 
124   glGetShaderiv(shader\_token, GL\_COMPILE\_STATUS, &shader\_ok);
125   \textcolor{keywordflow}{if} (!shader\_ok) \{
126     GLint maxLength = 0;
127     glGetShaderiv(shader\_token, GL\_INFO\_LOG\_LENGTH, &maxLength);
128 
129     \textcolor{comment}{//The maxLength includes the NULL character}
130     std::vector<char> errorLog(maxLength);
131     glGetShaderInfoLog(shader\_token, maxLength, &maxLength, &errorLog[0]);
132 
133     \textcolor{comment}{//Provide the infolog in whatever manor you deem best.}
134     std::cerr << \textcolor{stringliteral}{"Failed to compile "} << shader << \textcolor{stringliteral}{" with error code "} << shader\_ok << std::endl;
135     \textcolor{keywordflow}{for}(\textcolor{keyword}{auto} c: errorLog) \{
136       std::cerr << c;
137     \}
138 
139     glDeleteShader(shader\_token); \textcolor{comment}{//Don't leak the shader.}
140     exit(-1);
141   \}
142   \textcolor{keywordflow}{return} shader\_token;
143 \}
\end{DoxyCode}
...
\begin{DoxyCode}
83                                                                         \{
84   \textcolor{keyword}{auto} v\_shader\_token = \hyperlink{classGameAssetManager_a1a1e5c07f941e8d3fda40d9442ac7037}{CreateGLESShader}(GL\_VERTEX\_SHADER, vertex\_shader);
85   \textcolor{keyword}{auto} f\_shader\_token = \hyperlink{classGameAssetManager_a1a1e5c07f941e8d3fda40d9442ac7037}{CreateGLESShader}(GL\_FRAGMENT\_SHADER, fragment\_shader);
86 
87   GLint program\_ok;
88 
89   GLuint program = glCreateProgram();
90 
91   glAttachShader(program, v\_shader\_token);
92   glAttachShader(program, f\_shader\_token);
93   glLinkProgram(program);
94 
95   glGetProgramiv(program, GL\_LINK\_STATUS, &program\_ok);
96   \textcolor{keywordflow}{if} (!program\_ok) \{
97     std::cerr << \textcolor{stringliteral}{"Failed to link shader program:"} << std::endl;
98     glDeleteProgram(program);
99     exit(-1);
100   \}
101   \textcolor{keywordflow}{return} program;
102 \}
\end{DoxyCode}
...
\begin{DoxyCode}
69                             \{
70   \textcolor{keywordflow}{for}(\textcolor{keyword}{auto} ga: \hyperlink{classGameAssetManager_a671cddd92f1de4186c582fe0c4297b7d}{draw\_list}) \{
71     ga->Draw(\hyperlink{classGameAssetManager_ad7bab17862e06ca692289f934b40548b}{program\_token});
72   \}
73 \}
\end{DoxyCode}
\hypertarget{classGameAssetManager_a7c9e4fce50b47b78652e7ff0b4dbb629}{}\index{Game\+Asset\+Manager@{Game\+Asset\+Manager}!operator=@{operator=}}
...
\end{DoxyParamCaption}
)}\label{classGameAssetManager_a7c9e4fce50b47b78652e7ff0b4dbb629}
...
\begin{DoxyCode}
55                                                  \{
56   \textcolor{comment}{// TODO: implement this}
57 \}
\end{DoxyCode}
...
\begin{DoxyCode}
152                                                                         \{
153   std::ifstream input\_file;
154   GLint length;
155   input\_file.open(shader, std::ios::in);
156 
157   input\_file.seekg(0, std::ios::end);  \textcolor{comment}{// go to the end of the file}
158   length = input\_file.tellg();    \textcolor{comment}{// get length of the file}
159   input\_file.seekg(0, std::ios::beg);  \textcolor{comment}{// go to beginning of the file}
160 
161   GLchar * buffer = \textcolor{keyword}{new} GLchar[length+1];
162   input\_file.read(buffer, length);
163   buffer[length+1]=\textcolor{charliteral}{'\(\backslash\)0'};  \textcolor{comment}{// Ensure null terminated}
164 
165   input\_file.close();
166   \textcolor{keywordflow}{return} std::make\_pair(buffer, length);
167 \}
\end{DoxyCode}
